...
from libs.UNet_ser import UNet
...
if __name__ == '__main__':
    ...
    if args.profile == 'True':
        with profiler.profile(
            ...
            on_trace_ready=profiler.tensorboard_trace_handler(f'./log_spin_ser_{args.w}'),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        ) as p:
    ...
    if args.profile == 'True':
        with profiler.profile(
            ...
            on_trace_ready=profiler.tensorboard_trace_handler(f'./log_hd_ser_{args.w}'),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        ) as p:
    ...
    if args.trt=='True':
        ...
        print(f'||Unt_ser_size: {args.w} || Spin calc speed: {Spin_speed:.1e} s || Hd calc speed: {Hd_speed:.1e} s||')
